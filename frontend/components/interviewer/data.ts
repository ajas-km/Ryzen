...
// Full candidate object matching backend candidates.json structure
...
// Real candidates from candidates.json — full objects for API calls
export const BACKEND_CANDIDATES: BackendCandidate[] = [
  {
...
// Derive display-friendly candidate list from backend data
export function getCandidateDisplayList(): Candidate[] {
  return BACKEND_CANDIDATES.map((c) => {
    const { missionsFirstTry, missionsCompleted } = c.signals
    const matchScore = missionsCompleted > 0
      ? Math.round((missionsFirstTry / missionsCompleted) * 100)
      : 0

    let status: Candidate["status"] = "Screening"
    if (matchScore >= 70) status = "Qualified"
    else if (matchScore >= 40) status = "In Review"
    ...
    return {
      ...
      status,
      matchScore,
      date: `${c.member.yearsExperience}y exp · ${c.member.education}`,
    }
  })
}

// Look up full backend candidate by ID (for sending to API)
export function getBackendCandidate(id: string): BackendCandidate | undefined {
  return BACKEND_CANDIDATES.find((c) => c.member.id === id)
}
